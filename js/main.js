  ...
  initCriticalCam = function () {
        ...
        if (
          (1 == y.length &&
            (g.style.backgroundPosition =
              p + e.clientX - m + 'px ' + (v + e.clientY - u) + 'px'),
          2 == y.length)
        ) {
          var n =
            ((r = y[0].clientX),
            ...
            (l = x(r, c)),
            (s = x(i, a)),
            (d = Math.sqrt(Math.pow(l, 2) + Math.pow(s, 2))),
            d);
          f < n ? (q += 1) : (q -= 1),
            (g.style.backgroundSize = q + '%'),
            (f = n),
            (zoomText.textContent = q);
          var o =
            Math.atan2(
              y[1].clientY - y[0].clientY,
              y[1].clientX - y[0].clientX
            ) *
            (180 / Math.PI);
          h < o ? (S += 2) : (S -= 2),
            (g.style.filter = 'brightness(' + S + '%)'),
            (h = o),
            (brightText.textContent = S);
        }
        var r, i, c, a, l, s, d;
      },
      e = function (e) {
    ...
    g.addEventListener('pointerdown', function (e) {
      y.push(e),
        (m = e.clientX),
        (u = e.clientY),
        (p = parseInt(g.style.backgroundPositionX, 10)),
        (v = parseInt(g.style.backgroundPositionY, 10)),
        2 == y.length &&
          (h =
            Math.atan2(
              y[1].clientY - y[0].clientY,
      ...
      g.addEventListener('onpointerup', e);
  };
